...
export function parseThemePayload(input: string): { vars: ThemeVars; warnings: string[] } {
  ...
  if (payload.schemaVersion !== SCHEMA_VERSION) {
    warnings.push(
      `Unknown schema version: ${String(payload.schemaVersion)}. Tried best-effort import.`,
    );
  }
...
function isValidTokenValue(key: ThemeVarKey, value: string) {
  ...
  if (key === "--aria-font-sans") {
    return Object.values(fontOptions).includes(
      value as (typeof fontOptions)[keyof typeof fontOptions],
    );
  }
