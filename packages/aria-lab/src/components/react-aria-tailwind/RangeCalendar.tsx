...
import { Button } from "@/components/react-aria-tailwind/Button";
import { CalendarGridHeader } from "@/components/react-aria-tailwind/Calendar";
    ...
    >
      {Array.from({ length: months }, (_, i) => (
        <div key={i} className="@container flex flex-col w-[calc(9*var(--spacing)*7)]">
          ...
          <CalendarGrid
            offset={{ months: i }}
            className="[&_td]:px-0 [&_td]:py-px border-spacing-0"
          >
            <CalendarGridHeader />
            <CalendarGridBody>
              {(date) => (
