...
const chevronBase =
  "h-4 w-4 shrink-0 text-[var(--aria-text-secondary)] transition-transform duration-200";
  ...
  return (
    <AriaDisclosure
      className={composeClassName(
        className,
        () => "group border-b border-[var(--aria-border)] last:border-b-0",
      )}
      {...props}
    >
      {children}
...
export function DisclosurePanel({ children, className, ...props }: DisclosurePanelProps) {
  return (
    <AriaDisclosurePanel
      className={composeClassName(
        className,
        () => "overflow-clip transition-[height] duration-200",
      )}
      {...props}
    >
